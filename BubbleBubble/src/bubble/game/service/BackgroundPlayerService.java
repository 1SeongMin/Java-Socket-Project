...
public class BackgroundPlayerService implements Runnable{
	...
	public void run() {
		while(player.getState() == 0) {
			...
			// 2. 벽 충돌 체크
			// 색상 확인
			Color leftColor = new Color(image.getRGB(player.getX() - 10 + 15, player.getY() + 25 - 50));
			Color rightColor = new Color(image.getRGB(player.getX() + 50 + 15 + 15, player.getY() + 25 - 50));
			// -2가 나온다는 뜻은 바닥에 색깔이 없이 흰색
			int bottomColor = image.getRGB(player.getX() + 10 +15, player.getY() + 50 + 5 - 54) // -1
					+ image.getRGB(player.getX()+50 - 10 +15, player.getY() + 50 + 5 - 54); // -1
